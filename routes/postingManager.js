...
router.get('/', isAuthenticated, function(req, res, next) {
  Posting.find({
    ...
    res.render('allpostings', { user: req.user,
    	allpostings: postings, message: req.flash.message});
  });
});
...
router.get('/recommend', isAuthenticated, isProjectOwnder,  function(req, res, next) {
  Users.find({
    ...
    res.render('posting_detail', { user: req.user, recommend:bestMatch, 
    	thepost: posting});
  });
  });
});
	...
	Posting.findById(req.query.id, req.body, function (err, post) {
	    ...
	    post.save();
	     res.render('posting_detail', { user: req.user,
	    	thepost: post, message: req.flash('success', { msg: 'You have Joined this Project posting' })});
	  });
	});
...
router.get('/edit_posting', isAuthenticated, isProjectOwnder, function(req, res, next) {
	Posting.findById(req.query.id,  function (err, postings) {
	    if (err) return next(err);
	    //res.json(postings);
	    res.render('editPosting', { user: req.user,
	    	thepost: postings});
	  });
	});
...
router.post('/comment', isAuthenticated, function(req, res, next) {
	Posting.findById(req.body.id, function (err, post) {
	    ...
	    res.render('posting_detail', { user: req.user,
	    	thepost: post, message: req.flash('success', { msg: 'a new commented is posted' })});
	  });
	});
...
router.post('/createpost', isAuthenticated, isProjectOwnder, function(req, res, next) {
  ...
  newPost.save( function (err, post) {
    ...
    res.render('posting_detail', { user: req.user,
	    	thepost: newPost, message: req.flash('success', { msg: 'You have created this Project posting' })});
  });
});
